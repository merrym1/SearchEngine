...
st.caption("Query in Indonesian → Translate (NLLB) → Embed (LaBSE) → Retrieve (Cosine)")
...
if submitted:
    try:
        ...
        t0 = time.time()
        with st.spinner("Memuat engine… (sekali di awal)"):
        ...
        with st.spinner("Menerjemahkan…"):
            ...
            if isinstance(out_tuple, (tuple, list)) and len(out_tuple) >= 1:
                translated = out_tuple[0]
            else:
                raise RuntimeError("engine.query() tidak mengembalikan terjemahan.")
...
if out:
    ...
    st.subheader("Terjemahan Dalam Bahasa Batak Toba")
    ...
    if not df.empty:
        ...
        if not c_text or not c_score:
            ...
            for i, row in df.iterrows():
                ...
                st.markdown(title); st.write(txt); 
